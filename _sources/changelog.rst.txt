...
- **New Content Providers**
  ...
  - Add ``journals/`` umbrella package covering ``OJS`` (Open Journal Systems, detects the ``Open Journal Systems`` generator meta-tag) and ``Janeway`` (detects the ``application/geo+json`` alternate link emitted by the ``janeway_geometadata`` plugin), as metadata-only content providers for journal article landing pages. Both extract via a shared priority list — JSON-LD ``spatialCoverage`` first (richest geometry), then the ``geo+json`` alternate link, ``DC.SpatialCoverage`` GeoJSON / WKT, ``DC.box``, ISO 19139 EX_GeographicBoundingBox, OJS ``administrativeUnits`` fallback, and ICBM / ``geo.position`` as a last resort (points). Temporal extent uses the same fallback chain: JSON-LD ``temporalCoverage`` → ``DC.temporal`` / ``DC.PeriodOfTime`` ISO 8601 intervals → embedded GeoJSON ``temporal_periods`` → ``DC.Date.issued`` publication date. The existing ``Pensoft`` provider is folded into the same hierarchy (no external API change); ``GeoScienceWorld`` now imports the shared WKT parser from ``journals/_meta``. Article DOIs are lifted from the landing-page head (JSON-LD ``identifier``, ``citation_doi``, ``DC.Identifier``) and fed into ``--external-metadata`` enrichment so a user can pass a journal article URL (not just a DOI) and still get CrossRef / DataCite records (:issue:`76`)
