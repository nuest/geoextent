...
- Journal article landing-page support for [OJS](https://pkp.sfu.ca/ojs/) (with the [ojsGeo](https://github.com/nuest/ojsGeo) plugin), [Janeway](https://janeway.systems/) (with [janeway_geometadata](https://github.com/GeoinformationSystems/janeway_geometadata/)), [Pensoft](https://pensoft.net/), and [GeoScienceWorld](https://pubs.geoscienceworld.org/); reads JSON-LD `spatialCoverage`, Dublin Core `DC.SpatialCoverage` (GeoJSON / WKT), `DC.box`, ISO 19139 `EX_GeographicBoundingBox`, and ICBM / `geo.position` centroids — and lifts the article DOI out of the HTML head so `--ext-metadata` works on plain article URLs ([docs](https://nuest.github.io/geoextent/examples_journals.html))
