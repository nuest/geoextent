...
def test_invalid_coordinate_geojson_extract_bbox():
    result = geoextent.fromFile(
        "tests/testdata/geojson/invalid_coordinate.geojson", bbox=True
    )
...
def test_one_point_geojson_extract_bbox():
    ...
    assert result["bbox"] == pytest.approx(
        [6.220493, 50.521503, 6.220493, 50.521503], abs=tolerance
    )
    assert result["crs"] == "4326"
...
def test_geojson_extract_time():
    result = geoextent.fromFile(
        "tests/testdata/geojson/muenster_ring_zeit.geojson", tbox=True
    )
