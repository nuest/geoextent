...
class TestGeoTIFFTemporalExtent:
    ...
    def test_no_temporal_metadata(self):
        result = geoextent.from_file(
            "tests/testdata/tif/tif_no_temporal.tif", tbox=True
        )
        assert "tbox" not in result
    ...
    def test_acquisition_invalid_returns_none(self):
        result = geoextent.from_file(
            "tests/testdata/tif/tif_acq_invalid.tif", tbox=True
        )
        assert "tbox" not in result
...
class TestNetCDFTemporalExtent:
    ...
    def test_cf_days_since(self):
        result = geoextent.from_file("tests/testdata/nc/nc_days_since.nc", tbox=True)
        assert result["tbox"] == ["2015-01-01", "2016-01-01"]

    def test_cf_seconds_since(self):
        result = geoextent.from_file("tests/testdata/nc/nc_seconds_since.nc", tbox=True)
        assert result["tbox"] == ["2000-06-01", "2000-06-02"]

    def test_cf_minutes_since(self):
        result = geoextent.from_file("tests/testdata/nc/nc_minutes_since.nc", tbox=True)
        assert result["tbox"] == ["2010-12-25", "2010-12-26"]

    def test_acdd_coverage(self):
        result = geoextent.from_file("tests/testdata/nc/nc_acdd_coverage.nc", tbox=True)
    ...
    def test_cf_priority_over_acdd(self):
        result = geoextent.from_file("tests/testdata/nc/nc_cf_and_acdd.nc", tbox=True)
    ...
    def test_no_temporal_metadata(self):
        result = geoextent.from_file("tests/testdata/nc/nc_no_temporal.nc", tbox=True)
        assert "tbox" not in result
...
class TestRasterTemporalErrorHandling:
    ...
    def test_invalid_tifftag_logs_debug(self, caplog):
        with caplog.at_level(logging.DEBUG, logger="geoextent"):
            geoextent.from_file("tests/testdata/tif/tif_tifftag_invalid.tif", tbox=True)
        assert any(
