...
3. **Content Providers** (in `geoextent/lib/content_providers/`):
   ...
   - ``STAC`` (SpatioTemporal Asset Catalog) metadata-only provider for STAC Collections served by any STAC-compliant API; extracts pre-computed ``extent.spatial.bbox`` and ``extent.temporal.interval`` directly from Collection JSON; supports known STAC API hosts (Element84, DLR, Terradue, WorldPop, Lantmateriet, etc.), ``/stac/`` URL path patterns, and JSON content-inspection fallback; handles content negotiation for HTML/JSON servers and open-ended temporal ranges
   - ``NFDI4Earth`` (NFDI4Earth Knowledge Hub) metadata-only provider for the Cordra-based digital object repository (1.3M+ datasets, 168 repositories, 415K data services); extracts spatial extent (WKT geometry) and temporal ranges from the SPARQL endpoint with Cordra REST API fallback; supports OneStop4All landing page URLs (``onestop4all.nfdi4earth.de/result/{id}``) and direct Cordra object URLs; follows ``landingPage`` to other supported providers (disable with ``--no-follow``)
   - ``GitHub`` data-download provider for public GitHub repositories; uses Git Trees API (2 API calls per repo) and raw file downloads; supports repository root, branch, and subdirectory URLs; preserves directory structure for co-located files; ``GitHostProvider`` abstract base class enables future git hosting providers
   - ``GitLab`` data-download provider for public GitLab repositories on gitlab.com and self-hosted instances; uses paginated Repository Tree API and raw file API; supports nested namespace paths (``group/subgroup/project``), ``/-/tree/{ref}/{path}`` URLs, and ``.git`` suffixes; three-stage instance detection: known hosts (20+ research/government instances), hostname heuristic (``gitlab`` in hostname), and API probe fallback; optional ``GITLAB_TOKEN`` for higher rate limits
   - ``Forgejo`` data-download provider for public Forgejo and Gitea repositories (including Codeberg.org, Helmholtz DataHub, DataLad Hub); uses Gitea REST API v1 for paginated tree listing and raw file downloads; three-stage instance detection: known hosts, hostname heuristic (``forgejo`` or ``gitea`` in hostname), and ``/api/v1/version`` probe fallback; optional ``FORGEJO_TOKEN`` for higher rate limits
   - ``SoftwareHeritage`` data-download provider for the universal software archive (Inria + UNESCO); supports persistent SWHIDs (``swh:1:dir:...``, ``swh:1:ori:...``, ``swh:1:rev:...``, ``swh:1:cnt:...``, ``swh:1:snp:...``, ``swh:1:rel:...``), browse URLs (``archive.softwareheritage.org``), and SWHID qualifiers (``origin=``, ``path=``); resolves through API chain (origin → visit → snapshot → revision → directory); optional ``SWH_TOKEN`` for 1200 req/hr (120 anonymous)
   - ``RemoteRaster`` metadata-only provider for direct HTTP(S) URLs to GeoTIFF/COG files; reads raster headers via GDAL ``/vsicurl/`` without downloading the full file; matches URLs ending in ``.tif`` or ``.tiff``; positioned last in provider list as catch-all
