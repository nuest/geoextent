...
3. **Content Providers** (in `geoextent/lib/content_providers/`):
   ...
   - ``GitHub`` data-download provider for public GitHub repositories; uses Git Trees API (2 API calls per repo) and raw file downloads; supports repository root, branch, and subdirectory URLs; preserves directory structure for co-located files; ``GitHostProvider`` abstract base class enables future GitLab/Gitea providers
